...
public class DocumentCollection {
  ...
  public void match(String searchQuery) {//System.out.println("1");
    ...
    while (tmp != null) {
      tmp.setQuerySimilarity(tmp.getDocument().getWordCounts().computeSimilarity(queryDocument.getWordCounts(), this));
      tmp = tmp.getNext();
    }
  ...
  public int noOfDocumentContainingWord(String word)
  {
	  int number=0;
	  for(int i=0;i<this.numDocuments();i++)
	  {
		  if(this.get(i).getWordCounts().getIndexOfWord(word)!=-1)
		  {
			  int index=this.get(i).getWordCounts().getIndexOfWord(word);
			  if(this.get(i).getWordCounts().getCount(index)>=1)
			  {
				  number++;
			  }
		  }
