...
public class WordCountsArray {
   ...
   */
  public WordCount[] wordCounts;//xiugai
  ...
  private boolean wordsEqual(WordCountsArray wca) {
    ...
    if ((wca == null) || (this.size() != wca.size())) {
    	...
    	System.out.println("wca size:   "+wca.size());
      return false;
    }
    ...
    for (int i = 0; i < this.size(); i++) {
      if (!this.getWord(i).equals(wca.getWord(i))) {
    	  ...
    	  System.out.println("this:  "+this.getWord(i));
    	  System.out.println("wca:   "+wca.getWord(i));
        return false;
      }
    }
  ...
  {//System.out.println("1");
	  for(int i=0;i<this.size();i++)
	  {//System.out.println("1");
		  //System.out.println(dc.numDocuments());
		  //System.out.println("i:  "+i+"   "+dc.noOfDocumentContainingWord(this.getWord(i)));
		  
		  //double invertedFrequency = (Math.log((double)(dc.numDocuments()+1)/(double)(dc.noOfDocumentContainingWord(this.getWord(i)))));
		  //System.out.println("i:  (inverted)"+"   "+invertedFrequency);
		  //System.out.println("i:  numberofDC   "+dc.numDocuments());
		  
		  if(dc.noOfDocumentContainingWord(this.getWord(i))==0)
		  {
			  this.wordCounts[i].setWeight(0);
		  }
		  else {
		  
		  this.wordCounts[i].setWeight((double)this.getCount(i)*(Math.log((double)(dc.numDocuments()+1)/(double)(dc.noOfDocumentContainingWord(this.getWord(i))))));
		  //System.out.println("i:  "+i+"   "+this.wordCounts[i].getWeight());
	  }}
  }
  private void calculateNormalizedWeights(DocumentCollection dc)
  {
	  ...
	  for(int i=0;i<this.size();i++)
	  {
		  summe+=(double)this.wordCounts[i].getWeight()*(double)this.wordCounts[i].getWeight();
	  }
	  for(int i=0;i<this.size();i++)
	  {
		  this.wordCounts[i].setNormalizedWeight((double)this.wordCounts[i].getWeight()/(double)(Math.sqrt(summe)));
		 // System.out.println("i:  "+i+"   "+this.wordCounts[i].getNormalizedWeight());
	  }
  }
  private double scalarProduct(WordCountsArray wca,DocumentCollection dc)
  {//System.out.println("1");
	  if (wca == null) {System.out.println("1.1");
	      return 0;
	    }

	    /* scalar product is 0 by definition, if size is different */
	    if (this.size() != wca.size()) {System.out.println("1.2");
	      return 0;
	    }
	     ...
	     * wordsEqual()-check.
	     */
	    if ((this != wca) && !this.wordsEqual(wca)) {System.out.println("1.3");
	   /* for(int i=0;i<this.actualSize;i++)
	    {
	    	System.out.println(this.getWord(i));
	    }*/
	   
	    
	  /*  for(int i=0;i<this.actualSize;i++)
	    {
	    	System.out.println(wca.getWord(i));
	    }*/
	    ...
	    for(int i=0;i<this.size();i++)
	    {
	    	scalarProduct+=(double)this.wordCounts[i].getNormalizedWeight()*(double)wca.wordCounts[i].getNormalizedWeight();
	    }
	    return scalarProduct;
  }
  public double computeSimilarity(WordCountsArray wca,DocumentCollection dc)
  {
	return  this.scalarProduct(wca, dc);
  }
  /*public static void main(String[] args)
  {
	  DocumentCollection tmp =new DocumentCollection();
	  Document tmp2=new LinkedDocument(null,null,null,null,null,"alte die einmal eine es geiss hatte sieb war",null);
	  tmp.appendDocument(tmp2);
	
	  System.out.println(tmp.get(0).getWordCounts().getWord(2));
	  
	  WordCountsArray b= new WordCountsArray(6);
	  b.add("alte", 0);
	  b.add("die", 0);
	  b.add("ein", 0);
	  b.add("eine", 0);
	 ...
	 b.add("hatte", 0);
	  ...
	  b.add("ha", 1);
	  b.add("fick", 1);
	  System.out.println(tmp2.getWordCounts().computeSimilarity(b, tmp));
  }*/
